...
export async function POST(request, { params }) {
  try {
    ...
    // Save activity to student's activity history
...
export async function DELETE(request, { params }) {
  try {
    ...
    // Remove activity from student's activity history
    const updatedStudent = await Student.findOneAndUpdate(
      { _id: params.id, userId: verified.userId },
      { $pull: { activity_history: { _id: activityId } } },
      { new: true }
    );
    ...
    console.error('Error deleting activity from history:', error);
    return NextResponse.json(
      { error: error.message },
      { status: 500 }
    );
